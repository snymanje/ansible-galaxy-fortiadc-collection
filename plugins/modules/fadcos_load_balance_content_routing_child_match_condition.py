...
DOCUMENTATION = """
module: fadcos_load_balance_content_routing_child_match_condition
"""
...
obj_url = '/api/load_balance_content_routing_child_match_condition'
...
def main():
    argument_spec = dict(
        ...
        content=dict(type='str', default='match'),
        object=dict(type='str'),
        reverse=dict(type='str', default='disable'),
        type=dict(type='str', default='string'),
        ignorecase=dict(type='str', default='enable'),
        id=dict(type='str'),
    ...
    if not param_pass:
        ...
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            for entry in data['payload']:
                if 'id' in module.params.keys() and entry['mkey'] == module.params['id']:
