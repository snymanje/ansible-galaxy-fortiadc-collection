...
DOCUMENTATION = """
module: fadcos_load_balance_persistence_child_iso8583_bitmap
"""
...
obj_url = '/api/load_balance_persistence_child_iso8583_bitmap'
    ...
    if not param_pass:
        ...
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            for entry in data['payload']:
                if entry['mkey'] == module.params['id']:
                    result['changed'] = False
                    result['err_msg'] = "Duplicate entry."
                    found = True
                    break
        if found == False:
