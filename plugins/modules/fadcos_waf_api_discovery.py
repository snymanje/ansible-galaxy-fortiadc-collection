...
DOCUMENTATION = """
---
module: fadcos_waf_api_discovery
short_description: Manage FortiADC Web Application Firewall API discovery by RESTful API
description:
...
def get_waf_api_discovery(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_api_discovery'
    if name:
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_api_discovery(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_api_discovery(module, connection)
        result['res'] = response
    elif action == 'edit':
        ...
        if res:
            code, response = edit_waf_api_discovery(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_api_discovery(module, connection)
