...
DOCUMENTATION = """
---
module: fadcos_waf_api_gateway_user
short_description: Manage FortiADC Web Application Firewall API gateway users by RESTful API
description:
...
def get_waf_api_gateway_user(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_api_gateway_user'
    if name:
...
def main():
    ...
    if not param_pass:
        ...
        code, response = get_waf_api_gateway_user(module, connection)
        result['res'] = response
    elif action == 'edit':
        ...
        if res:
            code, response = edit_waf_api_gateway_user(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_api_gateway_user(module, connection)
