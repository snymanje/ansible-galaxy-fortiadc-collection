...
DOCUMENTATION = """
---
module: fadcos_waf_cors_headers
short_description: Manage FortiADC Web Application CORS header by RESTful API
description:
...
def add_waf_cors_headers(module, connection):
    payload = {
        'mkey': module.params['name'], 
    }

    url = '/api/security_waf_cors_headers'
...
def edit_waf_cors_headers(module, payload, connection):
    name = module.params['name']
    url = '/api/security_waf_cors_headers?mkey=' + name
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_cors_headers(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_cors_headers'
    if name:
...
def delete_waf_cors_headers(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_cors_headers?mkey=' + name
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_cors_headers(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_cors_headers(module, connection)
        result['res'] = response
    elif action == 'delete':
        code, data = get_waf_cors_headers(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_cors_headers(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
