...
DOCUMENTATION = """
---
module: fadcos_waf_cors_headers_child_list
...
def add_waf_cors_headers_child_cors_headers_list(module, connection):
    payload = {
        'header': module.params['header'],            
        }

    url = '/api/security_waf_cors_headers_child_cors_headers_list?pkey=' + module.params['name']
...
def edit_waf_cors_headers_child_cors_headers_list(module, payload, connection):
    pkey = module.params['name']
    mkey = module.params['id']
    url = '/api/security_waf_cors_headers_child_cors_headers_list?pkey=' + pkey + '&mkey=' + mkey
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_cors_headers_child_cors_headers_list(module, connection):
    pkey = module.params['name']
    mkey = module.params['id']
    payload = {}
    url = '/api/security_waf_cors_headers_child_cors_headers_list?pkey=' + pkey
    if mkey:
        url += '&mkey=' + mkey
...
def delete_waf_cors_headers_child_cors_headers_list(module, connection):
    pkey = module.params['name']
    mkey = module.params['id']
    payload = {}
    url = '/api/security_waf_cors_headers_child_cors_headers_list?pkey=' + pkey + '&mkey=' + mkey
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_cors_headers_child_cors_headers_list(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_cors_headers_child_cors_headers_list(module, connection)
        result['res'] = response
    elif action == 'edit':
        code, data = get_waf_cors_headers_child_cors_headers_list(module, connection)
        ...
        if res:
            code, response = edit_waf_cors_headers_child_cors_headers_list(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_cors_headers_child_cors_headers_list(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_cors_headers_child_cors_headers_list(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
