...
DOCUMENTATION = """
---
module: fadcos_waf_cors_protection
short_description: Manage FortiADC Web Application Firewall CORS protection by RESTful API
description:
...
def add_waf_cors_protection(module, connection):
    payload = {
        'mkey': module.params['name'], 
        'status': module.params['status'], 
    }

    url = '/api/security_waf_cors_protection'
...
def edit_waf_cors_protection(module, payload, connection):
    name = module.params['name']
    url = '/api/security_waf_cors_protection?mkey=' + name
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_cors_protection(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_cors_protection'
    if name:
...
def delete_waf_cors_protection(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_cors_protection?mkey=' + name
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_cors_protection(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_cors_protection(module, connection)
        result['res'] = response
    elif action == 'edit':
        code, data = get_waf_cors_protection(module, connection)
        ...
        if res:
            code, response = edit_waf_cors_protection(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_cors_protection(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_cors_protection(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
