...
DOCUMENTATION = """
---
module: fadcos_waf_data_leak_prevention_dictionary
short_description: Manage FortiADC Web Application Firewall data leak prevention dictionary by RESTful API
description:
...
def add_waf_data_leak_prevention_dictionary(module, connection):
    ...
    url = '/api/security_waf_dlp_dictionary'
...
def edit_waf_data_leak_prevention_dictionary(module, payload, connection):
    name = module.params['name']
    url = '/api/security_waf_dlp_dictionary?mkey=' + name
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_data_leak_prevention_dictionary(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_dlp_dictionary'
    if name:
...
def delete_waf_data_leak_prevention_dictionary(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_dlp_dictionary?mkey=' + name
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_data_leak_prevention_dictionary(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_data_leak_prevention_dictionary(module, connection)
        result['res'] = response
    elif action == 'edit':
        code, data = get_waf_data_leak_prevention_dictionary(module, connection)
        ...
        if res:
            code, response = edit_waf_data_leak_prevention_dictionary(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_data_leak_prevention_dictionary(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_data_leak_prevention_dictionary(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
