...
DOCUMENTATION = """
---
module: fadcos_waf_input_validation_policy
short_description: Manage FortiADC Web Application Firewall input validation policy by RESTful API
description:
...
- name:
  ...
  gather_facts: false
  tasks:
    - name: Add input_validation_policy
      fadcos_waf_input_validation_policy:
        action: add
        name: test_ivp1

    - name: get input_validation_policy
      fadcos_waf_input_validation_policy:
        action: get
        name: test_ivp1

    - name: delete input_validation_policy
      fadcos_waf_input_validation_policy:
        action: delete
        name: test_ivp1
"""
...
def add_waf_input_validation_policy(module, connection):
    payload = {
        'mkey': module.params['name'], 
    }

    url = '/api/security_waf_input_validation_policy'
...
def edit_waf_input_validation_policy(module, payload, connection):
    name = module.params['name']
    url = '/api/security_waf_input_validation_policy?mkey=' + name
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_input_validation_policy(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_input_validation_policy'
    if name:
...
def delete_waf_input_validation_policy(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_input_validation_policy?mkey=' + name
...
def main():
    ...
    if not param_pass:
        ...
        code, response = add_waf_input_validation_policy(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_input_validation_policy(module, connection)
        result['res'] = response
    elif action == 'delete':
        code, data = get_waf_input_validation_policy(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_input_validation_policy(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
