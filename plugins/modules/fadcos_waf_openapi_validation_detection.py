...
DOCUMENTATION = """
---
module: fadcos_waf_openapi_validation_detection
short_description: Manage FortiADC Web Application Firewall OpenAPI validation detection by RESTful API
description:
...
def add_waf_openapi_validation_detection(module, connection):
    payload = {
        'mkey': module.params['name'], 
        'action': module.params['security_action'],
        'exception_id': module.params['exception_id'],
        'openapi_format_checks': module.params['openapi_format_checks'],
        'openapi_schema_checks': module.params['openapi_schema_checks'],
        'severity': module.params['severity'],
    }

    url = '/api/security_waf_openapi_validation_detection'
...
def edit_waf_openapi_validation_detection(module, payload, connection):
    name = module.params['name']
    url = '/api/security_waf_openapi_validation_detection?mkey=' + name
    if is_vdom_enable(connection) and not is_global_admin(connection):
        vdom = module.params['vdom']
        url += '&vdom=' + vdom
...
def get_waf_openapi_validation_detection(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_openapi_validation_detection'
    if name:
...
def delete_waf_openapi_validation_detection(module, connection):
    name = module.params['name']
    payload = {}
    url = '/api/security_waf_openapi_validation_detection?mkey=' + name
...
def main():
    argument_spec = dict(
        ...
        openapi_format_checks=dict(type='str'),
        openapi_schema_checks=dict(type='str'),
    ...
    if not param_pass:
        ...
        code, response = add_waf_openapi_validation_detection(module, connection)
        result['res'] = response
        result['changed'] = True
    elif action == 'get':
        code, response = get_waf_openapi_validation_detection(module, connection)
        result['res'] = response
    elif action == 'edit':
        code, data = get_waf_openapi_validation_detection(module, connection)
        ...
        if res:
            code, response = edit_waf_openapi_validation_detection(module, new_data, connection)
            result['res'] = response
            result['changed'] = True
    elif action == 'delete':
        code, data = get_waf_openapi_validation_detection(module, connection)
        if 'payload' in data.keys() and data['payload'] and type(data['payload']) is not int:
            code, response = delete_waf_openapi_validation_detection(module, connection)
            result['res'] = response
            result['changed'] = True
        else:
            result['failed'] = False
